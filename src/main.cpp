...
{    
  ...
  if( osm_data.empty() && !osm_data_file.empty() ) {
    std::cout << "Reading OpenStreetMap datas from the following file: " <<  osm_data_file << std::endl;
  ...
  //	   For testing
//       RoutePlanner route_planner{model, 12, 13, 90, 90};
  RoutePlanner route_planner{model, start_x, start_y, end_x, end_y};
  route_planner.AStarSearch();
  std::cout << route_planner.GetDistance() << std::endl;
  Render render{model};
